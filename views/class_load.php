...
function getAllData(){
	...
	$("#myTable").DataTable({
		"paging":false,
		"ajax":{
			"url":"ajax/datatables/getAllClassLoad.php?class_id=<?= $get_class_id; ?>",
			"dataSrc":"data"
		},
		"columns":[
			{
...
function removeStu(id,name){
    swal({
        ...
        function(isConfirm){
            if (isConfirm) {
				$.post("ajax/deleteClassLoad.php",{
					id:id
				},function (data,status){
					// alert(data);
					if(data == 1){
				 		swal("Good!", "Student successfully removed!", "success");
						getAllData();
					}else if(data == 2){
						swal("Oops!", "Unable to remove! Student had taken quiz, !", "warning");
						getAllData();
					}else{
						error_delete();
					}
